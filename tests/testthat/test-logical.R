...
test_that("logical vector push_back and insert work", {
    lua("x = luajr.logical()")
    lua("for i = 1, 100 do x:push_back(i % 2 == 0 and R.TRUE or R.FALSE) end")
    expect_equal(lua("return #x"), 100)
    expect_equal(lua("return x[1]"), 0L)
    expect_equal(lua("return x[2]"), 1L)

    lua("y = luajr.logical({R.TRUE, R.FALSE, R.TRUE})")
    lua("y:insert(2, 2, R.FALSE)")
