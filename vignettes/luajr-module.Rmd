...
- [Additional types](#extra), including environments, R functions, matrices,
  and data frames.
- [Constants](#constants) defined by the `luajr` module.
- [Parallel processing](#parallel) features of the `luajr` module.
- [Utility functions](#utility) for `luajr`.
...
these bounds, as the `luajr` module does not do bounds checking. Going out of 
...
`v:clear()`) drop all structural attributes (`names`, `dim`, `dimnames`) 
consistently. If you need a result to keep matrix shape after such changes, 
re-attach `dim` via `v:set_attr("dim", ...)` before returning it.
...
a 'bare' value. If you are returning a single value to R, prefer to use this 
form as it will preserve `NA`s; for example, if 
...
otherwise. This is the recommended way to test for `NA` because direct
comparison with `luajr.NA_real_` always returns `false` (due to NaN semantics
in IEEE-754: `NaN == NaN` is false). For `luajr.numeric`, `v:is_na(i)` matches
R's `is.na()` semantics and returns `true` for both `NA_real_` and any other
`NaN` value. `v:is_na(i)` errors on `luajr.list`, since list elements are
themselves R values and have no per-element NA concept.
...
vector's existing storage in place when capacity allows, and only allocates a
new buffer if the new length would exceed the current capacity.
...
`luajr` can run Lua code in parallel across multiple worker Lua states. Each
worker is a fully independent Lua state. Because the vector types described
above depend on R, and R is not fully thread-safe, **do not allocate new R
objects from inside a worker** (do not create new `luajr.numeric`,
`luajr.list`, etc., and do not call R functions that allocate). Reading and
writing the elements of a vector that was *allocated in the main state* and
passed in to the worker is safe, provided different workers do not write to
the same slot — this is the standard pattern for collecting per-worker
results, illustrated below.

The following renders a *Buddhabrot* in parallel — a Monte Carlo image of the
Mandelbrot set built by sampling random points, iterating each one, and
accumulating the trajectories of those that escape. Each worker accumulates
into its own image buffer (to avoid write races on shared pixels), and the
main state sums them at the end. Then displays the result with R's `image()`:
...
buddhabrot <- lua_func("function(cfg)
    ...
    for t = 1, #bufs do
        local b = bufs[t]
        for k = 1, n do result[k] = result[k] + b[k] end
    end
    return result
end", "$.")
...
img <- buddhabrot(cfg)
image(-img, col = hcl.colors(256, "Oslo"),
...
Pulling the example apart:

- `luajr.workers()` creates a pool of one worker per core; `pool.n` reports
  the count.
- `pool:srun(setup)` runs `setup` in each worker once, in sequence — used
  here to give each worker a different RNG seed (via `thread_id`) so the
...
- `pool:close()` shuts the workers down.
- The main state sums the per-worker buffers into one result. `log1p()` on
  the R side compresses the long count tail before `image()` displays it.

**`luajr.ncores()`**

Returns a suggested number of concurrent threads for the current machine
(the value of `std::thread::hardware_concurrency()` from C++).

**`luajr.workers(n)`**

Constructs a pool of `n` worker Lua states. If `n` is `nil` or missing, the
default is `luajr.ncores()`. Each worker is created with the standard `luajr`
and Lua libraries already loaded.
...
within a single R `.Call` (i.e. within a single call from R into Lua):
cross-state argument transfer uses by-reference handles that are only valid
for that scope. Specifically, cdata arguments (vectors, environments, R
functions) carried into workers reference the calling state's R objects,
which R only keeps alive for the duration of the `.Call`; pointer-type cdata
...
worker by value (for plain Lua values) or by reference handle (for cdata, 
restricted to the calling `.Call` lifetime).
...
Same call shape as `srun`, but runs the workers concurrently on separate
threads. If any worker throws an error, the first error is propagated back to
the calling state after all workers complete.
...
These are general-purpose helpers exposed by the `luajr` module.
...
- `luajr` vector/environment/rfunction cdata &rarr; the underlying SEXP
