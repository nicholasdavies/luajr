...
`lua_func()` with args code `"v"`, but you can also create new vectors within
Lua. 
...
function using `lua_func()` with args code `"r"`, but you can also create new 
...
luajr.numeric_r(z)    -- Copied from z
```
...
sets the attribute to `val`. Note that even though you can get and manipulate
...
integers or with strings. Unlike Lua tables, a list remembers the order in 
...
Get or set the attribute named `attr`. The second form, with two arguments,
sets the attribute to `val`. Note that, for a list, the `"names"` attribute is 
...
This is because "under the hood", R's TRUE is defined as 1 (or any nonzero 
integer besides NA), FALSE is defined as 0, and NA (when logical or integer) is 
defined as a special 'flag' value of -2147483648 (i.e. -2^31). However, in Lua, 
anything other than `nil` or `false` evaluates as `true`, meaning that the 
following Lua code

```lua
x = luajr.logical({ luajr.TRUE, luajr.FALSE, luajr.NA_logical_ })
if x[1] then print("First element of x is TRUE!") end
```

may appear to work, but in fact it will print the statement about `x[1]` being 
TRUE regardless of whether `x[1]` is `luajr.TRUE`, `luajr.FALSE`, or 
`luajr.NA_logical_`.

When testing for NA in Lua, you have to check if a value is equal to (`==`) or
not equal to (`~=`) the corresponding NA constant above, depending on the type 
