...
|`list(...)`     |`list(1, b='b')`|`{[1]=1, b='b'}` |**error**        |**error**   |**error**        |`x = luajr.list(); x[1]=luajr.numeric({1}); x.b=luajr.character({'b'}); return x`|`x = luajr.list(); x[1]=luajr.numeric_r({1}); x.b=luajr.character_r({'b'}); return x`|
...
#### Underwhelming example: sum of squares
...
When the vector is 10,000,000 elements long, the story is similar. This is an
example where luajr doesn't add that much speed&mdash;the function is
relatively short, and there is a certain amount of overhead in invoking it, 
while the R function doesn't have the overhead of transferring control between 
languages.

#### More compelling example: logistic map
...
logistic_map_R = function(x0, burn, iter, A)
{
    ...
    for (a in A) {
        x = x0
        for (i in 1:burn) { x = a * x * (1 - x); }
        for (i in 1:iter) { result$x[j] = x; x = a * x * (1 - x); j = j + 1 }
    }
...
"function(x0, burn, iter, A)
    ...
    for k,a in pairs(A) do
        local x = x0
        for i=1,burn do  x = a * x * (1-x)  end
        for i=1,iter do  result.a[j] = a; result.x[j] = x; x = a * x * (1-x); j = j + 1  end
    end
...
bench::mark(
    data_R <- logistic_map_R(0.5, 500, 500, seq(2, 3.85, 0.05)),
    data_L <- logistic_map_L(0.5, 500, 500, seq(2, 3.85, 0.05))
)

sum(abs(data_R$x - data_L$x)) # If this is 0, results are exactly the same for both
```

The version written in Lua is around 2,000 times faster than the version in R.
