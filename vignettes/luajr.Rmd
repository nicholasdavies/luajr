...
|---------------------------------------------|-------------------|---------------:|
|`logistic_map_R(0.5, 50, 100, 200:385/100))` | R function        |       750 µs   |
|`logistic_map_L(0.5, 50, 100, 200:385/100))` | Lua function      |      **70 µs** |
|`logistic_map_C(0.5, 50, 100, 200:385/100))` | C++ function      |        90 µs   |

The version written in Lua is around 10 times faster than the version in R, 
and even somewhat outperforms `Rcpp`. Note that you could probably get the 
C++ version to go faster by not using `Rcpp`, as there is some overhead 
associated with parameter passing under `Rcpp`, but that is more advanced 
stuff. For example, I rewrote the C++ function above using `cpp11` instead
and it ran just marginally slower than the Lua version.
