...
be faster than C/C++, though usually it isn't quite as good. Why use luajr then? 
Rcpp and cpp11 require a C++ toolchain (e.g. gcc, clang, etc) and requires long 
compilation times, whereas luajr doesn't. This means that luajr is usable when
a C++ compiler isn't available and when compilation times are prohibitive or
an annoyance.
...
Here we are comparing two different methods (R versus Lua) of running a 
