 ...
 * `'x'`: pass as a bare SE**X**P (raw R object pointer)
...
The `'x'` arg code is not shown in the table above. For any R type, `'x'`
passes the value as a bare SEXP (i.e. the raw R object pointer). This bypasses
all type-specific conversions and is intended for use with the `R` Lua module
(loaded via `require("R")`).
...
f1 = lua_func("function(x) x[1][1] = 999; x:set('a', 42); end", ".")
f1(x)
print(x)
```
...
f3 = lua_func("function(x) x[1][1] = 999; x:set('a', 42); return x; end", ".")
x = f3(x)
print(x)

f4 = lua_func("function(x) x[1] = luajr.numeric({888, 999}); return x; end", ".")
x = f4(x)
print(x)
```
...
and data.frames -- in your Lua code, see the 
[R objects in Lua code](objects.html) vignette.
